#!/usr/bin/env bash
# Obtain a Microsoft Entra access token for a **human user** (delegated OAuth).
#
# The user must belong to an Entra security group that is assigned an app role
# on this API (e.g. System.Maintainer, System.Reader, Entitlement.Reader).
# The token's `roles` claim is what the API authorizes on.
#
# Prerequisites:
#   - APP_AZURE_TENANT_ID
...
Obtain a delegated access token for a human user against the Platform Management API.
...
Examples:
  ...
  # Export into current shell + show claims
  eval "\$(./scripts/get-token-human.sh --export --print-claims)"

  # Device code (no browser on this machine)
  export AZURE_HUMAN_CLIENT_ID=<spa-or-public-client-id>
  TOKEN=\$(./scripts/get-token-human.sh --method device-code)
